 ...
 *     PERSONAL: quien no alcanzó a entrar en la ventana no lo tiene.
 *
 * El cierre se deriva de la DURACIÓN de la clase (en LGS, 1 h → reconexión hasta
 * el minuto 50). Si mañana cambia la duración de un tipo, esto se ajusta solo.
 *
...
/**
 * Instantes clave de una clase, en milisegundos. La reconexión nunca puede
 * acortar la ventana de ingreso (`max`): en una clase muy corta `duración − 10`
 * caería antes del cierre normal.
 */
export function zoomLimites(inicioMs: number, tipo?: string | null) {
  const dur = eventDurationMin(tipo || undefined);
...
export function estadoZoom(
  ...
  const { abre, cierraIngreso, cierraReconexion } = zoomLimites(inicioMs, tipo);
...
export function zoomDisponible(
  inicioMs: number, tipo?: string | null,
  accesoEnMs?: number | null, ahoraMs: number = Date.now(),
): boolean {
  return estadoZoom(inicioMs, tipo, accesoEnMs, ahoraMs) === 'disponible';
}
...
export const MENSAJE_ZOOM_CERRADO = 'La clase ya terminó';
