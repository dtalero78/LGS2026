...
export default function AdvisorCalendar({ advisorId, advisorName }: AdvisorCalendarProps) {
  ...
  const loadEvents = async () => {
    try {
      ...
      console.error('Error loading advisor events:', error)
    } finally {
      setLoading(false)
    }
  }

  const loadInscripciones = async (eventIds: string[]) => {
    try {
      ...
      // Actualizar eventos con inscripciones y asistencias reales
      ...
      console.log('✅ Inscripciones actualizadas para eventos del advisor')
    } catch (error) {
