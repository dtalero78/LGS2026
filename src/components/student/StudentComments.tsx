...
export default function StudentComments({ studentId, usuarioId }: StudentCommentsProps) {
  ...
  // Para debugging, vamos a obtener el usuarioId desde la API de student
  const [realUsuarioId, setRealUsuarioId] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    if (studentId && !usuarioId) {
      fetchStudentData()
    }
  }, [studentId, usuarioId])

  const activeUsuarioId = usuarioId || realUsuarioId

  useEffect(() => {
    console.log('🔐 Session status:', status, 'Session data:', session)
    console.log('📊 StudentComments props:', { studentId, usuarioId })
    console.log('📊 Props types:', {
      studentIdType: typeof studentId,
      usuarioIdType: typeof usuarioId,
      studentIdValue: studentId,
      usuarioIdValue: usuarioId
    })
  }, [session, status, studentId, usuarioId])
  ...
  // Si no hay usuarioId, mostrar mensaje informativo
  if (!activeUsuarioId) {
    return (
      <div className="space-y-6 pb-32">
        <h3 className="text-lg font-medium text-gray-900">💬 Sistema de Comentarios</h3>
        <div className="text-center py-8 bg-blue-50 rounded-lg border border-blue-200">
          <p className="text-blue-700 font-medium mb-2">Comentarios no disponibles</p>
          <p className="text-blue-600 text-sm">
            Este estudiante no tiene un registro vinculado en el sistema de personas.
            <br />
            Solo los estudiantes con registro completo pueden tener comentarios.
          </p>
        </div>
      </div>
    )
