...
/**
 * POST /api/postgres/approvals/delete
 *   body: { contratos: string[], motivo?: string }
 *
 * Borra en CASCADA un contrato (titular + beneficiarios + FINANCIEROS +
 * PAGOS_TITULARES + ACADEMICA/bookings/overrides + KIDS_INSCRIPCIONES +
 * USUARIOS_ROLES), desde el Centro de Aprobaciones. GUARD: solo contratos
 * **NO APROBADOS** (rechaza `aprobacion = 'Aprobado'`). A diferencia de
 * matriculas/delete, SÍ permite borrar "Firmado sin aprobar" (contrato firmado
 * pero aún no aprobado). Cada contrato es una transacción atómica con snapshot
 * en PURGE_LOG (recuperable). Gateado por APROBACION.CENTRO.BORRAR
 * (SUPER_ADMIN/ADMIN bypass).
 */

interface DeleteResultItem {
  contrato: string;
  status: 'ok' | 'error' | 'ya_aprobado' | 'sin_titular';
  error?: string;
  borrados?: Record<string, number>;
}

export const POST = handlerWithAuth(async (request, _ctx, session) => {
  await requirePermission(session, AprobacionPermission.CENTRO_BORRAR);

  const body = await request.json().catch(() => ({}));
  const contratos = Array.isArray(body?.contratos) ? body.contratos : null;
  const motivo = (typeof body?.motivo === 'string' && body.motivo.trim())
    ? body.motivo.trim()
    : 'Borrado desde Centro de Aprobaciones (contrato no aprobado)';
  ...
  for (const contrato of contratos) {
    ...
    try {
      const outcome = await withTransaction(async (client) => {
        ...
        // GUARD: NO borrar contratos ya APROBADOS (defensa en profundidad — el
        // Centro solo lista no-aprobados, pero nunca se borra uno activo).
        if (String(titular.aprobacion || '').trim().toLowerCase() === 'aprobado') {
          return { skip: 'ya_aprobado' as const };
        }
        ...
        const complemSnap = academicaIds.length
          ...
          : { rows: [] };
        const kidsSnap = await client.query(`SELECT * FROM "KIDS_INSCRIPCIONES" WHERE "contrato" = $1`, [contrato]).catch(() => ({ rows: [] }));
        ...
        await client.query(
          `INSERT INTO "PURGE_LOG"
           ...
           VALUES ($1,'APROBACION_NO_APROBADO',$2,$3,$4,$5::jsonb,$6,$7,$8,$9,$10,$11::jsonb)`,
      ...
      else if ((outcome as any).skip === 'ya_aprobado') results.push({ contrato, status: 'ya_aprobado', error: 'El contrato ya está APROBADO; no se puede borrar desde el Centro de Aprobaciones' });
      else results.push({ contrato, status: 'ok', borrados: (outcome as any).borrados });
    } catch (err: any) {
      console.error(`[approvals/delete] ${contrato}:`, err?.message || err);
      results.push({ contrato, status: 'error', error: err?.message || 'Error desconocido' });
    }
  }

  const ok = results.filter(r => r.status === 'ok').length;
  const omitidos = results.filter(r => r.status === 'ya_aprobado').length;
  const failed = results.filter(r => r.status === 'error' || r.status === 'sin_titular').length;
  return successResponse({
    message: `Borrado: ${ok} OK · ${omitidos} omitidos (aprobados) · ${failed} fallidos`,
