    ...
    <DashboardLayout>
      <div className="space-y-5 pb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <CronCard title="Desbloqueo OnHold (reactivate-onhold)" schedule="Diario 03:00 UTC · 10 PM Colombia" h={data?.crons.reactivate ?? { lastRun: null, status: null, hoursSince: null, stale: true, processed: 0, success: 0, failed: 0, error: null }} />
          <CronCard title="Bloqueo por Vigencia (expire-contracts)" schedule="Diario 04:00 UTC · 11 PM Colombia" h={data?.crons.expire ?? { lastRun: null, status: null, hoursSince: null, stale: true, processed: 0, success: 0, failed: 0, error: null }} />
        </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {rows.map((r, i) => (
                  <tr key={`${r.studentId}-${i}`} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-gray-600">{(r as any)[extraCol] ?? '—'}</td>
