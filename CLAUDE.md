...
| Commit | Description |
|---|---|
| `local` | feat: **Informes > Contratos > Matrículas** — nuevo informe snapshot de contratos. "Contratos" pasa a ser submenú con "Matrículas" (nueva pestaña). Página [/dashboard/informes/contratos/matriculas](src/app/dashboard/informes/contratos/matriculas/page.tsx) + endpoint [route.ts](src/app/api/postgres/reports/contratos/matriculas/route.ts) (gateado por `INFORMES.CONTRATOS.MATRICULAS`). **6 tarjetas**: **Contratos x Aprobar** (TITULAR pendiente: `aprobacion` NULL o 'Pendiente', descartando nombres placeholder TITULAR/BENEFICIARIO o vacíos — excluye Rechazado/Devuelto/etc. ya decididos), **Vigentes** (`aprobacion IN (Aprobado,Aprobada)` Y `estado <> FINALIZADA`), **Finalizados** (`estado='FINALIZADA'`), **Beneficiarios** (de titulares vigentes vía `titularId`), **Académicos Activos** (ACADEMICA en Step 0–49 o WELCOME y `estadoInactivo!=true` — la finalización del titular marca estadoInactivo vía cascade), **Académicos Inactivos** (Step 50/DONE). **Barras** "pendientes por antigüedad" (1 sem–1 mes / 1–2 meses / +2 meses, desde `COALESCE(fechaContrato,_createdDate)`). **Dona** aprobadas-sin-finalizar vs sin-aprobar. **Heatmap** matrículas aprobadas país × mes del año en curso (mes de `COALESCE(inicioContrato,fechaContrato)`). Permiso `InformesPermission.CONTRATOS_MATRICULAS` registrado en catálogo (sección Contratos), middleware y pagePermissions. SQL validada contra producción (x Aprobar 373 · vigentes 2338 · finalizados 1488 · beneficiarios 3377 · acad. activos 3906 · inactivos 75) |
