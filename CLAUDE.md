...
- Key tables:
  ...
  - `USUARIOS_ROLES`: Credenciales y roles de usuario (email, password bcrypt/plain, rol, numeroId, contrato). Los campos `numeroId` y `contrato` se llenan al registrarse en `/nuevo-usuario/[id]` con datos de ACADEMICA; columnas auto-creadas con `ADD COLUMN IF NOT EXISTS`
