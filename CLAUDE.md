...
| Commit | Description |
|---|---|
| `local` | feat(conversión-titular): **proceso "Conversión Titular" — duplica un TITULAR como BENEFICIARIO y lo ubica en WELCOME** (`/dashboard/aprobacion/conversion-titular`). Hace lo mismo que marcar "Este titular será beneficiario" en el wizard de Crear Contrato, pero sobre un contrato ya existente. **Flujo (página, 3 estados)**: pide **número de contrato + número de identificación**; si coinciden con un TITULAR muestra sus datos (nombre, id, celular, correo, contrato, plataforma) y pide confirmación; al confirmar crea el beneficiario y muestra "Se creó el beneficiario X para el titular Y" + botón que redirige a **`/person/[titularId]?tab=administracion`** (queda listo para aprobar). **Qué hace en BD** (transacción `withTransaction`): (1) **duplica TODAS las columnas** del TITULAR en `PEOPLE` (82 cols vía `INSERT ... SELECT` con lista de columnas leída de `information_schema` — robusto ante cambios de esquema y conserva JSONB nativo sin round-trip JS), cambiando **solo `tipoUsuario` → BENEFICIARIO** (+ `_id` nuevo, `titularId` = _id del titular, fechas `NOW()`, casts `::text` en los overrides de la SELECT-list); el resto se copia verbatim (incluido `aprobacion`, que para un contrato recién creado es Pendiente → "listo para aprobar"). (2) **crea el registro en `ACADEMICA` en WELCOME** (`nivel='WELCOME'`, `step='WELCOME'`) para que pueda tomar clases. **No** crea `USUARIOS_ROLES` (login) ni `FINANCIEROS` — eso ocurre después en el flujo normal de aprobación/registro (igual que el wizard). El beneficiario se enlaza al titular por **`contrato` compartido**, así que aparece automáticamente en `/person/[titularId]` → Administración. **Idempotencia**: si ya existe un BENEFICIARIO con ese `numeroId`+`contrato`, no duplica (avisa "ya convertido"). **Arquitectura** (CLAUDE.md): repo `PeopleRepository.{findTitularByContratoAndNumeroId, existsBeneficiarioByNumeroIdAndContrato, duplicateTitularAsBeneficiario(client?)}` + `AcademicaRepository.createWelcomeRecord(client?)`; servicio `conversion-titular.service.ts` (`lookupTitular`, `convertirTitular`); endpoints `POST /api/postgres/aprobacion/conversion-titular/{lookup,convertir}` (`handlerWithAuth` + `requirePermission(CONVERSION_TITULAR_VER)`). Verificado: `tsc` limpio, rutas compilan (page 307, APIs 401), y la duplicación dinámica validada contra datos reales con `BEGIN/ROLLBACK` (82 cols OK, sin persistir). |
