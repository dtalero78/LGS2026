...
- **Contenido del PDF**: plantilla de arte por nivel (`public/certificados/{beginner,practical,functional}.png`) que **ya trae impreso "60 hours of instruction"**; el generador solo sobreimprime **nombre completo** (arriba, sobre la línea) y **fecha de aprobación del jump** (al pie, debajo de los cargos). Cifrado AES-128 con **clave = `ACADEMICA.numeroId`** del alumno (PDFKit, [src/lib/certificado-pdf.ts](src/lib/certificado-pdf.ts)). Las posiciones de los overlays son fracciones de página en la constante `POS` (fáciles de reajustar si cambia el arte).
...
- **Plantillas editables**: el arte es reemplazable desde **Recursos Académicos › Certificados** (`/dashboard/academic/actualizar-material/certificados`). `certificado-pdf.ts` (`loadCertificadoBg`) lee primero `certificados/{nivel}.png` de **DO Spaces** (plantilla personalizada) y si no existe usa el arte del repo (`public/certificados/`). API `GET/POST /api/postgres/materials/certificado-plantilla` (GET = estado + preview presigned; POST = reemplazar, **solo SUPER_ADMIN/ADMIN**, sube PNG a Spaces con ACL privada).
- **Límite de una generación (solo panel estudiante)**: desde el panel del estudiante cada certificado se puede generar **una sola vez** por alumno/nivel. Se registra en la tabla `CERTIFICADOS_GENERADOS` (auto-creada `CREATE TABLE IF NOT EXISTS`; UNIQUE `studentId`+`nivel`); el 2º intento devuelve `ValidationError` y en el modal el botón queda deshabilitado ("· generado"). `certificadoService.generar(id, nivel, {soloUna:true})` bloquea+registra; `getEstado(id, {incluirGenerado:true})` agrega el flag `yaGenerado`. El **panel admin (`/student/[id]`) NO tiene límite** (staff regenera cuantas veces quiera; no consume ni registra).
...
- Key tables:
  - `PEOPLE`: Personas (titulares y beneficiarios), contratos, OnHold, consentimiento declarativo, comentarios
    ...
    - Campo `reciboInscripcion` (JSONB nullable) — **recibo de pago de la inscripción del contrato**, INDEPENDIENTE de `documentacion`. Lo sube el comercial en el proceso de contrato con el botón **"Subir recibo inscripción"** (`GET/POST /api/contracts/[id]/recibo-inscripcion`): sube el archivo → lo lee con IA → guarda `{url,nombre,tipo,subidoPor,subidoEn,extraido:{medioPago,fecha,monto,referencia,banco,confianza}}` aquí y refleja los campos en `FINANCIEROS.recibo*` (best-effort). Gate: permiso `COMERCIAL.CONTRATO.SUBIR_RECIBO` + flag `leer_recibo_activo` (SUPER_ADMIN/ADMIN bypass); bloqueado en PRB-. Migración [scripts/add-recibo-inscripcion.js](scripts/add-recibo-inscripcion.js). Se muestra como columna "Recibo insc." en el Centro de Validación (pestaña Inscripción); el botón "Recibo" del gestor de recaudos se conserva
  ...
  - `ACADEMICA_BOOKINGS`: Inscripciones a eventos (asistencia, evaluación, calificación, participación, comentarios). Datos migrados de Wix usan columna `idEvento` (nueva: `eventoId`) y `tipoEvento` (queries usan COALESCE para compatibilidad). Columna `confirmadoExamen` (BOOLEAN, nullable) = estado de la inscripción a un examen internacional en Agrupación (SetUp Ciclo): `true`=Confirmado, `false`/NULL=Pendiente; Cancelado se representa con `cancelo=true` (soft-cancel que libera el cupo)
  - `CALENDARIO`: Eventos (SESSION, CLUB) con advisor, nivel, step, linkZoom, limiteUsuarios. Eventos de bienvenida se distinguen por `tituloONivel=WELCOME`. La columna `tipo=WELCOME` existe solo en datos legacy de Wix. Columna `cicloId` (VARCHAR, nullable) marca los eventos generados por un ciclo de Exam. Intern. (ver `EXAM_CICLOS`). Columnas `horaFin` (VARCHAR "HH:mm", nullable) y `duracionMin` (INTEGER, nullable) — duración programada del evento; hoy las escriben las sesiones de Exam. Intern. según la duración de la franja (1 o 2 h) y el registro de sesión pre-carga el Time Out con `horaFin` ([scripts/add-duracion-calendario.js](scripts/add-duracion-calendario.js))
  ...
  - `FINANCIEROS`: Datos financieros (totalPlan, pagoInscripcion, saldo, cuotas, formaPago). `fechaPago` (timestamptz) = **fecha del corte de pago** (su día del mes es el día de cuota); la usa el **Estado de Cuenta** para calcular el calendario de cuotas (cuota N vence = `fechaPago + (N-1) meses`). **Estado de Cuenta**: botón en el Resumen Financiero del titular (permiso `PERSON.FINANCIERA.ESTADO_CUENTA`) → `GET /api/postgres/pagos-titulares/estado-cuenta?idPeople=&modo=mes|total&mes=YYYY-MM` genera un **PDF cifrado con contraseña = número de documento del titular** (AES-128 vía `pdfkit`, [src/lib/estado-cuenta-pdf.ts](src/lib/estado-cuenta-pdf.ts)), con logo (`public/logo.png`), firma del Jefe de Recaudos por país (Chile vs resto, de `USUARIOS_ROLES` rol `RECAUDOS_JEFE`) y movimientos cruzados con los pagos **registrados** (validados o no) de `PAGOS_TITULARES`. `pdfkit` va externalizado en `next.config.js` (`serverComponentsExternalPackages`) para no romper la carga de sus fuentes AFM en runtime. El PDF incrusta la **firma manuscrita** del jefe según país desde `public/firmas/` (Chile → `ernesto-rodriguez.png`; resto → `nidian-poveda.png`) e incluye un indicador **"Progreso de Pago"** (barra segmentada: Inscripción + una por cuota, coloreada por estado, con leyenda contada + Valor del plan/Pagado/Saldo/Progreso%). En la pestaña Financiera, junto a **"Corte de Pago"** (día del mes de `fechaPago`) se muestra un badge del período actual: **En tiempo** (azul) / **Pagado** (verde) / **En mora** (rojo). **⚠️ Gotcha de importes**: las columnas `numeric` (`PAGOS_TITULARES.valorPagado/descuento/saldo`) las devuelve `pg` como **string con decimales** (`"214500.00"`) — al parsear montos usar `parseFloat` respetando el punto, **nunca** quitar todos los no-dígitos (eso convertía `"214500.00"` en 21.450.000). **Reconciliación**: cuenta **TODOS** los pagos de `PAGOS_TITULARES` (validados o no) y la deuda se cubre con **`valorPagado + descuento`** (`saldo = total − pagado − descuento`, `progreso = cobertura/total`). **Asignación por cuota**: cada pago **empieza a cubrir en SU `numCuota` y desborda hacia adelante** (adelantos / cambio a contado que liquida varias cuotas), **nunca hacia atrás** — así el estado por cuota coincide con el detalle de pagos: un abono parcial deja **su** cuota en estado **Parcial** (no mueve el parcial a la última), y un pago de la cuota 9 no marca la 8. El PDF agrega una fila "Descuento aplicado" y una cifra Descuento cuando hay descuento, y la barra distingue Pagada/Parcial/En mora/Por vencer. ⚠️ El Saldo del Estado de Cuenta puede diferir del "Saldo a la Fecha" del panel: `FINANCIEROS.saldo` solo cuenta pagos **validados**, mientras el Estado de Cuenta cuenta todos los registrados. Colores corporativos azules (logo LGS); la lista de personas se rotula **"BENEFICIARIOS"**
  ...
  - `EXAM_CICLOS`: Ciclos de exámenes internacionales (funcionalidad **SetUp Ciclo y Agrupación**, en Servicio › Exam. Intern.). Campos: `_id`, `nombre` (ej. "Octubre2026"), `fechaInicial`/`fechaFinal` (DATE), `config` (JSONB: por examen IELTS/TOEFL/B2FIRST un array de franjas `{dias:[0-6, 0=Dom], hora, advisor, linkZoom, cupo, duracion}` — `duracion` en horas: 1 o 2), `diasEspeciales` (JSONB array de decisiones sobre festivos/días off — `{fecha, tipo:'FESTIVO'|'OFF', accion:'OMITIR'|'MOVER', fechaReemplazo?}`, [scripts/add-diasespeciales-exam-ciclos.js](scripts/add-diasespeciales-exam-ciclos.js)), `estado` (BORRADOR/GENERADO), `eventosGenerados`, `creadoPor`. Creada por [scripts/create-exam-ciclos-table.js](scripts/create-exam-ciclos-table.js) (idempotente). En el SetUp, el panel **"Festivos y días off"** cruza las fechas que generarían sesión con los festivos de **CL/CO/EC/PE** ([src/lib/festivos.ts](src/lib/festivos.ts)) y permite, por día completo, **Omitir** o **Mover** (default: generar igual). Al "Generar eventos" se crean sesiones en `CALENDARIO` (nivel=examen, step 47/48/49, fecha/hora en America/Bogotá, con `horaFin`+`duracionMin` según la duración de la franja — columnas nuevas [scripts/add-duracion-calendario.js](scripts/add-duracion-calendario.js)) marcadas con `cicloId`, en una transacción, **honrando `diasEspeciales`** (OMITIR = no crea ese día; MOVER = lo crea en `fechaReemplazo`); un ciclo GENERADO no se edita/regenera directo (candado de idempotencia). Para editarlo se **reabre** (`POST .../exam-ciclo/[id]/reabrir` → borra sus eventos y vuelve a BORRADOR) y para eliminarlo **Borrar** (`DELETE .../exam-ciclo/[id]` → borra ciclo + eventos); ambas ops bloquean (409) si el ciclo tiene inscritos activos (`countInscritosCiclo`). Lógica en [src/services/exam-ciclo.service.ts](src/services/exam-ciclo.service.ts). Los "confirmados" que aparecen en Agrupación salen de `EXAM_INTERN_AUDIT` (accion='EXTENDIDO'); un estudiante con >1 `prueba` distinta ahí = colisión (aparece en ambos programas). En Agrupación el **agendamiento es por CURSO** (toda la serie del examen: N franjas + todas las fechas del ciclo), no por evento suelto; el estado por inscripción (Confirmado/Pendiente/Cancelado) aplica a toda la serie. Al **Aplicar Confirmación** el beneficiario pasa a `PEOPLE.estado='EXAM. INTER.'` (óvalo amarillo en [EstadoBadge](src/components/common/EstadoBadge.tsx)) y revierte a `FINALIZADA` al vencer el contrato (cron `expire-contracts` / bloqueo en el panel). Las sesiones de examen (IELTS/TOEFL/B2FIRST) **no se pueden cancelar** desde el panel del estudiante (guarda en `cancelBooking` de [student-booking.service.ts](src/services/student-booking.service.ts) + UI con candado). Permisos: `SERVICIO.EXAM_INTERN.SETUP_VER` / `SETUP_GENERAR` / `AGRUPACION_AGENDAR`
  - `EXAM_INTERN_AUDIT`: Auditoría del flujo "Aplicar Confirmación" de Exam. Intern. (IELTS/B2 First/TOEFL). Auto-creada (`CREATE TABLE IF NOT EXISTS`) en [exam-intern.service.ts](src/services/exam-intern.service.ts) y asegurada también por el script de ciclos. Campos: `_id`, `studentId` (ACADEMICA._id), `numeroId`, nombres, `email`, `celular`, `prueba`, `accion` ('EXTENDIDO'|'BLOQUEADO'), `fechaBase`, `nuevoFinalContrato`, `vigenciaAnterior`, `whatsappEnviado`, `ejecutadoPor`, `_createdDate`. Es la fuente de la lista de confirmados en Agrupación. **Regla de vigencia (CONFIRMADOS)**: el modal "Aplicar Confirmación" exige elegir el **ciclo** y la nueva `finalContrato` = **`EXAM_CICLOS.fechaFinal + 7 días`** (misma para todos los del ciclo), no `fechaBase + 100` (ese quedó como fallback histórico si no llega `cicloId`; `fechaBase` ahora solo alimenta el mensaje de WhatsApp de inicio de sesiones). Constante `DIAS_POST_CICLO=7` en [exam-intern.service.ts](src/services/exam-intern.service.ts)
...
| Exam. Intern. — SetUp Ciclo y Agrupación | `/dashboard/servicio/exam-intern/setup-ciclo` | SERVICIO.EXAM_INTERN.SETUP_VER |
