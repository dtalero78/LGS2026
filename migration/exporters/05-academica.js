/**
 * Academica Collection Exporter
 * Exports academic records from Wix to PostgreSQL
 *
 * Expected records: ~30K+
 * Priority: ALTA (student academic history)
 */
...
class AcademicaExporter {
  constructor() {
    this.collectionName = 'ACADEMICA';
    this.wixEndpoint = `${config.wix.baseUrl}${config.wixEndpoints.ACADEMICA}`;
    this.pgTable = config.pgTables.ACADEMICA;
    this.batchSize = config.batching.ACADEMICA;
    this.rateLimit = config.rateLimit.ACADEMICA;
    this.jsonbFields = config.jsonbFields.ACADEMICA;
  }
...
if (require.main === module) {
  (async () => {
    ...
    try {
      ...
      // Run export
      const exporter = new AcademicaExporter();
...
module.exports = AcademicaExporter;
