/**
 * USUARIOS_ROLES Collection Exporter
 * Exports user authentication data from Wix to PostgreSQL
 *
 * Expected records: ~50-100
 * Priority: ALTA (authentication system)
 */
...
class UsuariosRolesExporter {
  constructor() {
    this.collectionName = 'USUARIOS_ROLES';
    this.wixEndpoint = `${config.wix.baseUrl}${config.wixEndpoints.USUARIOS_ROLES}`;
    this.pgTable = config.pgTables.USUARIOS_ROLES;
    this.batchSize = config.batching.USUARIOS_ROLES;
    this.rateLimit = config.rateLimit.USUARIOS_ROLES;
    this.jsonbFields = config.jsonbFields.USUARIOS_ROLES;
  }
...
if (require.main === module) {
  (async () => {
    ...
    try {
      ...
      // Run export
      const exporter = new UsuariosRolesExporter();
...
module.exports = UsuariosRolesExporter;
