/**
 * ROL_PERMISOS Collection Exporter
 * Exports role definitions and permissions from Wix to PostgreSQL
 *
 * Expected records: ~9
 * Priority: ALTA (needed before USUARIOS_ROLES)
 */
...
class RolPermisosExporter {
  constructor() {
    this.collectionName = 'ROL_PERMISOS';
    this.wixEndpoint = `${config.wix.baseUrl}${config.wixEndpoints.ROL_PERMISOS}`;
    this.pgTable = config.pgTables.ROL_PERMISOS;
    this.batchSize = config.batching.ROL_PERMISOS;
    this.rateLimit = config.rateLimit.ROL_PERMISOS;
    this.jsonbFields = config.jsonbFields.ROL_PERMISOS;
  }
...
if (require.main === module) {
  (async () => {
    ...
    try {
      ...
      // Run export
      const exporter = new RolPermisosExporter();
...
module.exports = RolPermisosExporter;
