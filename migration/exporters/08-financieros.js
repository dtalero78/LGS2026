/**
 * Financieros Collection Exporter
 * Exports financial data from Wix to PostgreSQL
 *
 * Expected records: ~3-5K
 * Priority: MEDIA (financial records)
 */
...
class FinancierosExporter {
  constructor() {
    this.collectionName = 'FINANCIEROS';
    this.wixEndpoint = `${config.wix.baseUrl}${config.wixEndpoints.FINANCIEROS}`;
    this.pgTable = config.pgTables.FINANCIEROS;
    this.batchSize = config.batching.FINANCIEROS;
    this.rateLimit = config.rateLimit.FINANCIEROS;
    this.jsonbFields = config.jsonbFields.FINANCIEROS;
  }
  ...
  transformRecord(wixRecord) {
    ...
    // Clean all date fields specific to FINANCIEROS
    record.fechaPago = cleanDate(record.fechaPago);
    record.fechaUltimoPago = cleanDate(record.fechaUltimoPago);
...
if (require.main === module) {
  (async () => {
    ...
    try {
      ...
      // Run export
      const exporter = new FinancierosExporter();
...
module.exports = FinancierosExporter;
