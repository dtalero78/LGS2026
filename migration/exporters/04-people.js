/**
 * PEOPLE Collection Exporter
 * Exports student and titular data from Wix to PostgreSQL
 *
 * Expected records: ~5-10K
 * Priority: ALTA (core data, needed before ACADEMICA)
 */
...
class PeopleExporter {
  constructor() {
    this.collectionName = 'PEOPLE';
    this.wixEndpoint = `${config.wix.baseUrl}${config.wixEndpoints.PEOPLE}`;
    this.pgTable = config.pgTables.PEOPLE;
    this.batchSize = config.batching.PEOPLE;
    this.rateLimit = config.rateLimit.PEOPLE;
    this.jsonbFields = config.jsonbFields.PEOPLE;
  }
  ...
  transformRecord(wixRecord) {
    ...
    // Handle documentacion JSONB field (may not be in config)
    if (record.documentacion !== undefined && record.documentacion !== null) {
      if (typeof record.documentacion === 'object') {
        record.documentacion = JSON.stringify(record.documentacion);
      } else if (typeof record.documentacion === 'string') {
        try {
          JSON.parse(record.documentacion);
        } catch {
    ...
    record.fechaNacimiento = cleanDate(record.fechaNacimiento);
    record.fechaContrato = cleanDate(record.fechaContrato);
    record.fechaIngreso = cleanDate(record.fechaIngreso);
    record.inicioContrato = cleanDate(record.inicioContrato);
    record.finalContrato = cleanDate(record.finalContrato);
    record.fechaOnHold = cleanDate(record.fechaOnHold);
    record.fechaFinOnHold = cleanDate(record.fechaFinOnHold);
   ...
   * Sends all records in a single PG query for performance
   */
  buildBatchUpsertQuery(transformedRecords) {
    // Find union of all column names across all records
  ...
  async export(pool, options = {}) {
    ...
    while (hasMore) {
      ...
      if (!dryRun) {
        // Load PG columns once to filter out unknown Wix fields
        if (!this.pgColumns) {
...
if (require.main === module) {
  (async () => {
    ...
    try {
      ...
      // Run export
      const exporter = new PeopleExporter();
