/**
 * NIVELES Collection Exporter
 * Exports level definitions from Wix to PostgreSQL
 *
 * Expected records: ~20
 * Priority: MEDIA (catalog table, needed before PEOPLE/ACADEMICA)
 */
...
class NivelesExporter {
  constructor() {
    this.collectionName = 'NIVELES';
    this.wixEndpoint = `${config.wix.baseUrl}${config.wixEndpoints.NIVELES}`;
    this.pgTable = config.pgTables.NIVELES;
    this.batchSize = config.batching.NIVELES;
    this.rateLimit = config.rateLimit.NIVELES;
    this.jsonbFields = config.jsonbFields.NIVELES;
  }
...
module.exports = NivelesExporter;
