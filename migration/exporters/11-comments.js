/**
 * Comments Collection Exporter
 * Exports comments from Wix to PostgreSQL
 *
 * Expected records: Variable
 * Priority: BAJA (comments)
 */
...
class CommentsExporter {
  constructor() {
    ...
    this.wixEndpoint = `${config.wix.baseUrl}${config.wixEndpoints.COMMENTS}`;
    this.pgTable = config.pgTables.COMMENTS;
    this.batchSize = config.batching.COMMENTS;
    this.rateLimit = config.rateLimit.COMMENTS;
    this.jsonbFields = config.jsonbFields.COMMENTS;
  }
...
if (require.main === module) {
  (async () => {
    ...
    try {
      ...
      // Run export
      const exporter = new CommentsExporter();
...
module.exports = CommentsExporter;
