/**
 * Clubs Collection Exporter
 * Exports club definitions from Wix to PostgreSQL
 *
 * Expected records: ~20-30
...
class ClubsExporter {
  constructor() {
    ...
    this.wixEndpoint = `${config.wix.baseUrl}${config.wixEndpoints.CLUBS}`;
    this.pgTable = config.pgTables.CLUBS;
    this.batchSize = config.batching.CLUBS;
    this.rateLimit = config.rateLimit.CLUBS;
    this.jsonbFields = config.jsonbFields.CLUBS;
  }
...
if (require.main === module) {
  (async () => {
    ...
    try {
      ...
      // Run export
      const exporter = new ClubsExporter();
