/**
 * StepOverrides Collection Exporter
 * Exports step overrides from Wix to PostgreSQL
 *
 * Expected records: Variable
 * Priority: BAJA (overrides)
 */
...
class StepOverridesExporter {
  constructor() {
    this.collectionName = 'STEP_OVERRIDES';
    this.wixEndpoint = `${config.wix.baseUrl}${config.wixEndpoints.STEP_OVERRIDES}`;
    this.pgTable = config.pgTables.STEP_OVERRIDES;
    this.batchSize = config.batching.STEP_OVERRIDES;
    this.rateLimit = config.rateLimit.STEP_OVERRIDES;
    this.jsonbFields = config.jsonbFields.STEP_OVERRIDES;
  }
...
if (require.main === module) {
  (async () => {
    ...
    try {
      ...
      // Run export
      const exporter = new StepOverridesExporter();
...
module.exports = StepOverridesExporter;
