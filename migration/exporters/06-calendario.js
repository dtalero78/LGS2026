/**
 * Calendario Collection Exporter
 * Exports calendar events and sessions from Wix to PostgreSQL
 *
 * Expected records: ~5-10K
 * Priority: ALTA (needed before ACADEMICA_BOOKINGS)
 */
...
class CalendarioExporter {
  constructor() {
    this.collectionName = 'CALENDARIO';
    this.wixEndpoint = `${config.wix.baseUrl}${config.wixEndpoints.CALENDARIO}`;
    this.pgTable = config.pgTables.CALENDARIO;
    this.batchSize = config.batching.CALENDARIO;
    this.rateLimit = config.rateLimit.CALENDARIO;
    this.jsonbFields = config.jsonbFields.CALENDARIO;
  }
  ...
  transformRecord(wixRecord) {
    ...
    // This is the main field that stores when the event occurs
    if (record.dia) {
      // Wix already stores 'dia' in UTC format, just ensure it's ISO string
...
module.exports = CalendarioExporter;
