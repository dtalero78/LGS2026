 ...
 * add-rubro-column.js
 *
 * Utilidad: agrega `PEOPLE.rubro` (VARCHAR 150, nullable) — giro o actividad
 * económica del contratante. Solo se captura y se muestra cuando el titular es
 * EMPRESA (`tipoPersona='Empresa'`); en persona natural queda NULL.
 *
 * En el contrato aparece dentro del bloque {{datosTitular}} de la variante
 * Empresa, entre "NIT / RUT" y "Domicilio".
 *
 * Uso:
 *   node scripts/add-rubro-column.js            (dry-run)
 *   node scripts/add-rubro-column.js --apply    (escribe)
 *
  ...
  try {
    ...
    const { rows: existe } = await client.query(
      `SELECT column_name FROM information_schema.columns
        WHERE table_name = 'PEOPLE' AND column_name = 'rubro'`
    );
    if (existe.length) {
      console.log('La columna PEOPLE.rubro ya existe — nada que hacer.');
      return;
    }
    ...
    console.log('');
    console.log('DDL: ALTER TABLE "PEOPLE" ADD COLUMN IF NOT EXISTS "rubro" VARCHAR(150)');
    ...
    await client.query('ALTER TABLE "PEOPLE" ADD COLUMN IF NOT EXISTS "rubro" VARCHAR(150)');
