#!/usr/bin/env node
/**
 * add-replegal-columns.js — agrega a PEOPLE las columnas del representante legal
 * (solo aplican a titulares tipoPersona='Empresa'):
 *   replegal    (VARCHAR) — nombre completo del representante legal
 *   replegalid  (VARCHAR) — identificación del representante legal
 *   replegalcel (VARCHAR) — celular del representante legal
 *
 * Idempotente (ADD COLUMN IF NOT EXISTS). Dry-run por defecto. Gestiona el firewall solo.
 * USO: node scripts/add-replegal-columns.js [--apply] [--no-fw]
 */
...
const COLS = ['replegal', 'replegalid', 'replegalcel'];
...
(async () => {
  ...
  try {
    ...
    await c.connect();
    console.log(`\n===== add-replegal-columns (${APPLY ? 'APPLY' : 'DRY-RUN'}) =====`);

    const existing = new Set((await c.query(
      `SELECT column_name FROM information_schema.columns WHERE table_name='PEOPLE' AND column_name = ANY($1)`, [COLS])).rows.map(r => r.column_name));
    const faltan = COLS.filter(x => !existing.has(x));
    console.log(`Existen: ${[...existing].join(', ') || '(ninguna)'}`);
    console.log(`Faltan : ${faltan.join(', ') || '(ninguna)'}`);

    if (!faltan.length) { console.log('\n✅ Nada que hacer, las 3 columnas ya existen.'); return; }
    if (!APPLY) { console.log(`\n[dry-run] usa --apply para crear: ${faltan.join(', ')}`); return; }

    for (const col of faltan) {
      await c.query(`ALTER TABLE "PEOPLE" ADD COLUMN IF NOT EXISTS "${col}" VARCHAR`);
      console.log(`  + ${col}`);
    }
    console.log(`\n🟢 Columnas creadas: ${faltan.join(', ')}`);
  } catch (e) {
